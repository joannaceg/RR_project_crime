...
Since there is `r sum(df$Homicide <= 0)` observations for which *Homicide* is non-positive, we do not need to add any number to it inside the logarithm.
...
In Box-Cox we obtain that $\lambda$ = `r bestNormalize::boxcox(df$Homicide)$lambda %>% round(4)` 
...
We can apply Box-Cox transformation also for some other variables and check whether any transformation should be considered.
...
We transform it exactly in the same way it was done in the Bachelor thesis. Later in replication we will consider also other transformation of this variable.
