...
model_diagnostic <- function(model, data = data, sig.level = 0.05) {
  ...
  return(result %>% 
           kbl(booktabs = T) %>%
           kable_material_dark(full_width = F, bootstrap_options = c("hover"), color = "white",
                               font_size = 20)
  )
}
...
knitr::include_graphics("model_selection_schema.png")
```
...
stargazer(model_Bachelor.final, model1.final, model2.final, model3.final, 
          ...
          column.labels = c("Bachelor","Reproduction","Replication_1", "Replication_2"), no.space = T)
...
hyp <- data.frame(H = c("Income inequality", "GDP per capita", "The level of education (basic)", "The level of education (tertiary)", 
                  ...
                  Bachelor_thesis = c(TRUE,TRUE,FALSE,TRUE,TRUE,FALSE,TRUE),
                  Reproduction = c(FALSE,TRUE,TRUE,TRUE,FALSE,TRUE,TRUE),
                  Replication_1 = c(FALSE,TRUE,TRUE,TRUE,FALSE,TRUE,TRUE),
                  Replication_2 = c(FALSE,TRUE,TRUE,TRUE,FALSE,TRUE,TRUE)
                  )
